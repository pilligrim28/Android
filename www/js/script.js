...
//const switchCircleTheme = switchTheme.querySelector(".switch-circle");
...
const handleChangeSwitchTheme = (event) => {
...
const removeTodo = (id) => {
  const isAgree = confirm("Вы уверены?");
  if (isAgree) {
    TodosService.remove(id);
    Notification.show("Заметка удалены", 1000);
    renderTodos(todos);
    Modal.close();
  }
};
...
const init = () => {
  ...
  tabbarItems.forEach((tabbarItem) =>
    tabbarItem.addEventListener("click", handleClickTabbarItem)
  );
